...
export default function IdentityScreen() {
  const { patientProfile, setPatientProfile, sosContacts, setSosContacts } = useContext(AppContext);
  ...
  const [form, setForm] = useState({
    ...
    contact1_name: sosContacts[0]?.name || '',
    contact1_phone: sosContacts[0]?.phone || ''
  });
  ...
  const handleSave = async () => {
    ...
    const updatedContacts = [...sosContacts];
    if (updatedContacts.length > 0) {
      updatedContacts[0] = { name: form.contact1_name, phone: form.contact1_phone };
    } else {
      updatedContacts.push({ name: form.contact1_name, phone: form.contact1_phone });
    }
