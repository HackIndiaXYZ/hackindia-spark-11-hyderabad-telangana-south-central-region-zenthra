...
export default function AlertScreen() {
  const { liveData, dismissAlert, triggerSOS, language, sosContacts } = useContext(AppContext);
  ...
  useEffect(() => {
    ...
    const startAlert = async () => {
      ...
      const lang = language === 'తెలుగు' ? 'te-IN' : (language === 'हिंदी' ? 'hi-IN' : 'en-US');
      Speech.speak(msg, {
        language: lang,
        rate: 0.9,
        pitch: 1.0
...
const styles = StyleSheet.create({
  ...
  statsBox: {
    ...
    ...Platform.select({
      ...
      web: {
        boxShadow: '0px 4px 8px rgba(239, 68, 68, 0.1)'
      }
    })
  },
  ...
  callBtn: { 
    ...
    ...Platform.select({
      ...
      web: {
        boxShadow: '0px 6px 12px rgba(239, 68, 68, 0.4)'
      }
    })
  },
